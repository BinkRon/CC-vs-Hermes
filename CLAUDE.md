...
4. **领活先绑需求 ID（契约 8）**：推进交付物的 session，在 `docs/backlog.csv` 选一个 `依赖` 全 `done` **且 `状态=draft`（无人认领）**的需求，**领取即把 `状态` 切出 draft 占位**、`handoff.md` 起一棒，本 session 只推进这一个；接**在制品**则读该 ID 最新一棒续做。纯讨论/流程 session 不绑。**独立需求可由不同 session 并行**（有依赖的按序；共享工作树时并合订本那步串行，契约 8.6）。
...
| **`docs/handoff.md`** | **接棒台账** | 跨 session 交接；每需求 ID 只留最新一棒（契约 8）|
| `docs/progress.md` | **运行态** | 每次干活**前看 / 后更新**（契约 4：与 backlog + handoff 同步）|
...
4. **过三道门**（见下）→ 修正 → 更新 `progress.md` → `python3 build.py` 并入合订本。
...
4. **设计门（2026-07-04 起，DSN-01）**：按 `docs/design-principles.md` §5 checklist 过（10 秒重点测试/三档配额/粗体标题抽查/两图差异 3 秒测试）。前三道保真实，这道保表达。
...
| 变化 | 更新哪个 |
|---|---|
| 领活开工 | 先绑 `backlog.csv` 一个需求 ID，`handoff.md` 起一棒（契约 8）|
| 完成一篇 / 进度推进 / 交棒 | `progress.md` + `backlog.csv`（状态）+ `handoff.md`（最新一棒）三处同步（契约 4/8：不同步不算完成）|
| 一篇彻底完成、结论定稿 | 把该章一句话结论写进 `archive.md`；该需求从 `backlog.csv` 移走；`progress.md` 章节清单只留状态+文件名 |
| 规则/流程改变 | `contract.md` |
