...
### INTRO-01 · 全书导读章（gating·待 PM 设计门 · 第 1 棒 · 2026-07-17）
- **指针**：`findings/00-intro.html`（新章·合订本首章，`build.py` CHAPTERS 已加 `00 导读`）· backlog `INTRO-01` · thesis＝导读只给地图与问法（对象与支点/两轴怎么来/两把仪器/章节地图/方法与边界），**不给任何一章结论**。
- **已完成（第 1 棒）**：视觉脚本 `docs/scratch/intro-01/visual-script.md` → 成稿（重档＝两轴地图手工 HTML：内圈实线四侧循环＋外圈虚线补集三分；中档＝章节地图行表与侧栏逐字咬合；制品＝Hermes README 首句 README.md:19 已直核逐字属实）→ 机械两关过（`node --check` OK·build 11 章 OK）→ 浏览器 smoke 过（0 console error·ch-00 默认落地·两轴地图/toc 渲染正确·01 章 stepper 1/6→2/6 可点·mermaid 深色）→ **独立剧透/失真门过**（1 必修已修＝08 双仪器例外被抹平，补回「坐在两轴接缝上两把仪器都用」；2 建议已修＝切片位置改「章末」、计算侧材料改「散在轴一各章」；零剧透/README 引文/章节地图方向/受众合规全 ✅）→ 修后 rebuild+复验 0 error。
- **三条纪律**（PM 讨论定稿 2026-07-17，已执行）：①轴二结构锚**补集三分法**不押「五关系」计数（SYN2-R 改五→四+尾声不伤导读）；②零剧透——章节地图只列问题，总变量独占综合②；③头号假设当公开事实前提给。
- **下一步**：交 PM ①设计门（`design-principles.md` Part 2.2·设计眼归 PM）②浏览器 smoke 验收。认可后 done 迁档两处（backlog 置 done·删本块·一句话结论沉 archive）。**SYN2-R 落地后回看章节地图与轴二措辞（预期一段话级）**。
- **阻塞**：设计门＋PM smoke 待 PM。
